...
class AIPlayer:
    ...
    def get_intelligent_move(self, state: Tuple[np.array, Dict[int, Integer]]) -> Tuple[int, bool]:
        ...
        while max_depth < 20:
            if (time.time() - start_time > self.time - 0.2):
            ...
            print(f"depth {max_depth} best move {best_move} best val {max_val} ")
            max_depth += 2

            # if (np.sum(state[0]) < (len(state[0])*(len(state[0][0])*1.5*0.65))): break
